...
const Home = () => {
  ...
  const features = user?.role === 'employer' ? [
    {
      icon: <FiCpu className="w-8 h-8" />,
      title: 'فرز المتقدمين بذكاء',
      description: 'نظام متطور يحلل السير الذاتية ويعطيك أفضل المرشحين بناءً على متطلباتك.'
    },
    {
      icon: <FiBriefcase className="w-8 h-8" />,
      title: 'إدارة شاملة للوظائف',
      description: 'لوحة تحكم احترافية لمتابعة إعلاناتك الوظيفية وتتبع المتقدمين بسهولة.'
    },
    {
      icon: <FiMessageCircle className="w-8 h-8" />,
      title: 'مقابلات فيديو مدمجة',
      description: 'لا حاجة لبرامج خارجية، أجرِ المقابلات مع المرشحين مباشرة من منصتنا.'
    },
    {
      icon: <FiTrendingUp className="w-8 h-8" />,
      title: 'تحليلات التوظيف',
      description: 'احصل على إحصائيات دقيقة حول تفاعل الكفاءات مع إعلاناتك الوظيفية.'
    }
  ] : [
    {
      icon: <FiCpu className="w-8 h-8" />,
      ...
      <section className="relative pt-12 pb-12 md:pt-32 md:pb-24 overflow-hidden px-4 md:px-0">
        ...
        >
          <motion.div variants={itemVariants}>
            ...
            >
              {user?.role === 'employer' ? 'وظّف أفضل الكفاءات' : t('home.hero_title')} <br className="hidden sm:block"/>
              <span className="premium-gradient-text drop-shadow-md">{user?.role === 'employer' ? 'بذكاء وسرعة' : t('home.hero_title_accent')}</span>
            </motion.h1>
            <motion.p 
              className="text-lg md:text-3xl mb-8 md:mb-14 themed-text-sec max-w-4xl mx-auto font-bold leading-relaxed opacity-90"
              variants={itemVariants}
            >
              {user?.role === 'employer' ? 'انشر وظائفك، أدر المتقدمين، ودع الذكاء الاصطناعي يختار لك أفضل المرشحين لاحتياجات شركتك.' : t('home.hero_subtitle')}
            </motion.p>
            <motion.div 
