...
const AIChatbot = () => {
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      text: `مرحباً ${user?.name || ''}! أنا مساعدك الذكي. كيف يمكنني مساعدتك في رحلة التوظيف اليوم؟`,
      sender: 'ai',
      timestamp: new Date()
    }
  ]);
  ...
  return (
    <div className={`fixed bottom-6 right-6 w-96 max-h-[600px] bg-white rounded-3xl shadow-premium z-50 flex flex-col overflow-hidden transition-all duration-300 ${isMinimized ? 'h-20' : 'h-[600px]'}`}>
      {/* Header */}
      <div className="bg-gradient-to-r from-primary-600 to-accent p-4 flex items-center justify-between text-white shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] font-bold opacity-80">نشط الآن</span>
            </div>
          </div>
        </div>
              ...
              <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`flex gap-2 max-w-[85%] ${msg.sender === 'user' ? 'flex-row-reverse' : ''}`}>
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                    ...
                    {msg.sender === 'user' ? <FaUser size={12} /> : <FaMagic size={12} />}
                  </div>
